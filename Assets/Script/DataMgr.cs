...
public class DataMgr
{
    ...
    private List<uint[,]> terrain = null;
    ...
    public void DeserializeTerrainData(Object obj)
    {
        ...
        if (asset != null)
        {
            terrain = ByteUtils.DeSerializable(asset.bytes) as List<uint[,]>;
        }
    }

    public uint GetUVIndexByPosition(int index,Vector2 position)
    {
        Vector2 pos = position + (mapAreaSize/2);
        int pos_x = Mathf.FloorToInt(pos.x/2);
        int pos_y = Mathf.FloorToInt(pos.y/2);
        uint uvIndex = terrain[index][pos_x, pos_y];
        return uvIndex;
    }
}
